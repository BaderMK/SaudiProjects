...
app.post("/process",formValidation, (request,response) =>{
    ...
    let msg = "";
    if(!errors.isEmpty()) {
        msg = "<h1>Sorry,we found validation errors with your sumbission </h1>"+
        "<p> you can try again <a href='reg.html'>  here <a/></p>"+"These are the validation errors: ";
    ...
    msg = "<h1>Thank you. your account was created.</h1>"+
        ...
        loginUser(email, password, (err, result) => {
            if (err) {
                ...
                if (result.length > 0) {
                    ...
                    response.send("<h1>تم تسجيل الدخول بنجاح</h1> <p> .للذهاب الى الصفحة الرئيسية<a href='index.html'> اضغط هنا <a/></p>");
                } else {
